...
for lr in param_grid["learning_rate"]:
    for depth in param_grid["depth"]:
        for iters in param_grid["iterations"]:
            ...
            print(
                f"lr={lr:<5} depth={depth:<2} iters={iters:<4} | "
...
model.save_model(f'./hyperparameter_tuning/output/catboost_model_seed_{SEED}.cbm')

with open(f'./hyperparameter_tuning/output/catboost_params_seed_{SEED}', "wb") as f:
    pickle.dump(best_params, f)
    
#%% Reproduce model
SEED = 23

model_path = f'./hyperparameter_tuning/output/catboost_model_seed_{SEED}.cbm'
model = CatBoostRegressor()
